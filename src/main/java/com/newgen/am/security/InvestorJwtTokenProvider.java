...
@Component
public class InvestorJwtTokenProvider {
    ...
    @PostConstruct
    protected void init() {
        secretKey = Base64.getEncoder().encodeToString(ConfigLoader.getMainConfig().getString(Constant.INVESTOR_JWT_SECRET).getBytes());
        validityInMilliseconds = ConfigLoader.getMainConfig().getLong(Constant.INVESTOR_JWT_EXPIRATION, 3600000);
    }
...
//    claims.put("auth", roles.stream().map(s -> new SimpleGrantedAuthority(s.getAuthority())).filter(Objects::nonNull).collect(Collectors.toList()));
    ...
    public Authentication getAuthentication(String token) {
        UserDetails userDetails = userService.checkAccessTokenByInvestorUser(getUsername(token), token);
