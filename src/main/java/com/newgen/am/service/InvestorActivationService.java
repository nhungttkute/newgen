...
public class InvestorActivationService {
	...
	public void activateInvestor(HttpServletRequest request, String approvalId, InterestRateDTO interestRateDto,
			long refId) {
			...
			if (Utility.setApprovalIDonRedis(redisTemplate, approvalId, refId)) {
				...
				if (Constant.STATUS_PENDING.equals(invActivationApproval.getStatus())) {
					...
					if (AuthorityUtils
							...
							.contains(invActivationApproval.getFunctionCode())) {
						...
						if (Utility.isCQGSyncOn()) {
							...
							if (cqgInfo != null) {
								...
								if (Utility.isNotNull(investorDto.getCqgInfo())
										&& Utility.isNotNull(investorDto.getCqgInfo().getCustomerId())) {
									customerId = investorDto.getCqgInfo().getCustomerId();
								} else {
									// create cqg customer
									CQGResponseObj customerRes = cqgService.createCQGCustomer(investorDto,
											cqgInfo.getProfileId(), refId);
									if (customerRes != null) {
										customerId = customerRes.getData().getCustomerId();
									} else {
										AMLogger.logMessage(className, methodName, refId,
												String.format(
														"Activating investor %s. Error: timeout creating customer",
														investorDto.getInvestorCode()));
										throw new CustomException(ErrorMessage.CQG_INFO_CREATED_UNSUCCESSFULLY,
												HttpStatus.OK);
									}
								}
								...
								String accountId = "";
								if (Utility.isNotNull(investorDto.getCqgInfo())
										&& Utility.isNotNull(investorDto.getCqgInfo().getAccountId())) {
									accountId = investorDto.getCqgInfo().getAccountId();
								} else {
									if (Utility.isNotNull(customerId)) {
										// create cqg account
										CQGResponseObj accountRes = cqgService.createCQGAccount(cqgInfo.getProfileId(),
												Utility.getCQGAccountName(investorDto), investorCode, customerId,
												refId);
										if (accountRes != null) {
											...
											AMLogger.logMessage(className, methodName, refId,
													String.format(
															"Activating investor %s. Error: timeout creating account",
															investorDto.getInvestorCode()));
											throw new CustomException(ErrorMessage.CQG_INFO_CREATED_UNSUCCESSFULLY,
													HttpStatus.OK);
										}
								...
								if (Utility.isNotNull(accountId)) {
									...
									String traderId = ConfigLoader.getMainConfig()
											.getString(Constant.CQG_CMS_TRADER_ID);
									boolean isAccAuthListSuccessful = cqgService.updateCQGAccountAuthList(
											Utility.convertStringToLong(accountId), traderId, refId);
									if (!isAccAuthListSuccessful) {
										AMLogger.logMessage(className, methodName, refId, String.format(
												"Activating investor %s. Error: timeout creating account authorization list",
												investorDto.getInvestorCode()));
										throw new CustomException(ErrorMessage.CQG_INFO_CREATED_UNSUCCESSFULLY,
												HttpStatus.OK);
									}
									...
									boolean isUpdateRiskParamsSuccessful = cqgService.updateCQGRiskParams(accountId,
											investorDto.getMarginMultiplier(), investorDto.getOrderLimit(),
											investorDto.getDefaultPositionLimit(), refId);
									if (!isUpdateRiskParamsSuccessful) {
										AMLogger.logMessage(className, methodName, refId, String.format(
												"Activating investor %s. Error: timeout setting account risk params",
												investorDto.getInvestorCode()));
										throw new CustomException(ErrorMessage.CQG_INFO_CREATED_UNSUCCESSFULLY,
												HttpStatus.OK);
									}
								...
								AMLogger.logMessage(className, methodName, refId,
										String.format("Activating investor %s. Error: Member %s doesnt have cqgInfo",
												investorDto.getInvestorCode(), investorDto.getMemberCode()));
						...
						invActivationApproval
								.setMarginSurplusInterestRate(interestRateDto.getMarginSurplusInterestRate());
						invActivationApproval
								.setMarginDeficitInterestRate(interestRateDto.getMarginDeficitInterestRate());
						...
						if (Utility.isNotifyOn()) {
							Utility.sendCreateNewUserEmail(Constant.INVESTOR_USER_PREFIX, "",
									investorDto.getInvestorCode(), password, pin, refId);
						}

						// get redis user info
						UserInfoDTO userInfo = Utility.getRedisUserInfo(redisTemplate, Utility.getAccessToken(request),
								refId);
						// send activity log
						activityLogService.sendActivityLog(userInfo, request,
								ActivityLogService.ACTIVITY_REACTIVATE_INVESTOR,
								ActivityLogService.ACTIVITY_REACTIVATE_INVESTOR_DESC, investorDto.getInvestorCode(),
								approvalId);
					} else {
	...
	private void updateInvestor(MongoDatabase database, String investorCode, InterestRateDTO interestRateDto,
			long refId) {
	...
	public void reactivateInvestor(HttpServletRequest request, String approvalId, long refId) {
		String methodName = "reactivateInvestor";
		try {
			if (Utility.setApprovalIDonRedis(redisTemplate, approvalId, refId)) {
				InvestorActivationApproval invActivationApproval = invActivationApprovalRepo.findById(approvalId).get();
				if (Constant.APPROVAL_STATUS_REJECTED.equals(invActivationApproval.getStatus())) {
					// check authorization
					if (AuthorityUtils
							.authorityListToSet(SecurityContextHolder.getContext().getAuthentication().getAuthorities())
							.contains(invActivationApproval.getFunctionCode())) {
						// get redis user info
						UserInfoDTO userInfo = Utility.getRedisUserInfo(redisTemplate, Utility.getAccessToken(request), refId);
						// send activity log
						activityLogService.sendActivityLog(userInfo, request, ActivityLogService.ACTIVITY_CREATE_INVESTOR,
								ActivityLogService.ACTIVITY_CREATE_INVESTOR_DESC, invActivationApproval.getInvestorCode(), approvalId);
						
						invActivationApproval.setStatus(Constant.STATUS_PENDING_ACTIVATE);
						invActivationApproval.setLastModifiedUser(userInfo.getUsername());
						invActivationApprovalRepo.save(invActivationApproval);
					} else {
						throw new CustomException(ErrorMessage.ACCESS_DENIED, HttpStatus.FORBIDDEN);
					}
