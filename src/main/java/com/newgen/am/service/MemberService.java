...
public class MemberService {
		...
		try {
			...
			List<? extends Bson> pipeline = Arrays
					.asList(new Document().append("$match", getQueryDocument(searchCriteria, userInfo)),
							new Document().append("$sort", sortQuery),
							new Document().append("$project",
									new Document().append("_id", 0.0).append("code", 1.0).append("name", 1.0)
											.append("status", 1.0).append("note", 1.0)
	...
	public void createMember(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			MemberDTO memberDto = Utility.getGson().fromJson(pendingApproval.getPendingData().getPendingValue(),
					MemberDTO.class);
			...
			activityLogService.sendActivityLog(userInfo, request, ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_DESC, memberDto.getCode(),
	...
	private void createMasterMemberUser(HttpServletRequest request, MemberDTO memberDto, String memberCode,
			Document masterUserRole, long refId) {
		...
		if (!existedUser) {
			try {
				...
				if (Utility.isNotifyOn()) {
					Utility.sendCreateNewUserEmail(Constant.MEMBER_MASTER_USER_PREFIX, "", username, password, pin,
							refId);
				}
			} catch (Exception e) {
				AMLogger.logError(className, methodName, refId, e);
	...
	private LoginAdminUser createLoginAdminUser(String memberCode, String memberName, UserDTO memberUserDto,
	...
	public void updateMember(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			...
			UpdateMemberDTO memberDto = Utility.getGson().fromJson(pendingApproval.getPendingData().getPendingValue(),
			...
			if (Utility.isNotNull(memberDto.getCompany())) {
				if (Utility.isNotNull(memberDto.getCompany().getName()))
					updateMember.append("company.name", memberDto.getCompany().getName());
				if (Utility.isNotNull(memberDto.getCompany().getTaxCode()))
					updateMember.append("company.taxCode", memberDto.getCompany().getTaxCode());
				if (Utility.isNotNull(memberDto.getCompany().getAddress()))
					updateMember.append("company.address", memberDto.getCompany().getAddress());
				if (Utility.isNotNull(memberDto.getCompany().getPhoneNumber()))
					updateMember.append("company.phoneNumber", memberDto.getCompany().getPhoneNumber());
				if (Utility.isNotNull(memberDto.getCompany().getFax()))
					updateMember.append("company.fax", memberDto.getCompany().getFax());
				if (Utility.isNotNull(memberDto.getCompany().getEmail()))
					updateMember.append("company.email", memberDto.getCompany().getEmail());

				if (Utility.isNotNull(memberDto.getCompany().getDelegate())) {
					if (Utility.isNotNull(memberDto.getCompany().getDelegate().getFullName())) {
						updateMember.append("company.delegate.fullName",
								memberDto.getCompany().getDelegate().getFullName());
					...
					if (Utility.isNotNull(memberDto.getCompany().getDelegate().getBirthDay()))
						updateMember.append("company.delegate.birthDay",
								memberDto.getCompany().getDelegate().getBirthDay());
					if (Utility.isNotNull(memberDto.getCompany().getDelegate().getIdentityCard()))
						updateMember.append("company.delegate.identityCard",
								memberDto.getCompany().getDelegate().getIdentityCard());
					if (Utility.isNotNull(memberDto.getCompany().getDelegate().getIdCreatedDate()))
						updateMember.append("company.delegate.idCreatedDate",
								memberDto.getCompany().getDelegate().getIdCreatedDate());
					if (Utility.isNotNull(memberDto.getCompany().getDelegate().getIdCreatedLocation()))
						updateMember.append("company.delegate.idCreatedLocation",
								memberDto.getCompany().getDelegate().getIdCreatedLocation());
					...
					if (Utility.isNotNull(memberDto.getCompany().getDelegate().getPhoneNumber())) {
						updateMember.append("company.delegate.phoneNumber",
								memberDto.getCompany().getDelegate().getPhoneNumber());
						updateMember.append("users.$.phoneNumber",
								memberDto.getCompany().getDelegate().getPhoneNumber());
						updateMember.append("contact.phoneNumber",
								memberDto.getCompany().getDelegate().getPhoneNumber());
					...
					if (Utility.isNotNull(memberDto.getCompany().getDelegate().getAddress()))
						updateMember.append("company.delegate.address",
								memberDto.getCompany().getDelegate().getAddress());
					if (Utility.isNotNull(memberDto.getCompany().getDelegate().getScannedFrontIdCard()))
						updateMember.append("company.delegate.scannedFrontIdCard",
								memberDto.getCompany().getDelegate().getScannedFrontIdCard());
					if (Utility.isNotNull(memberDto.getCompany().getDelegate().getScannedBackIdCard()))
						updateMember.append("company.delegate.scannedBackIdCard",
								memberDto.getCompany().getDelegate().getScannedBackIdCard());
					if (Utility.isNotNull(memberDto.getCompany().getDelegate().getScannedSignature()))
						updateMember.append("company.delegate.scannedSignature",
								memberDto.getCompany().getDelegate().getScannedSignature());
				}
			}
	...
	public void updateMemberPA(HttpServletRequest request, String memberCode, ApprovalUpdateMemberDTO memberDto,
			long refId) {
	...
	private String insertMemberUpdatePA(UserInfoDTO userInfo, String memberCode, ApprovalUpdateMemberDTO memberDto,
			long refId) {
	...
	public MemberDTO getMemberDetail(HttpServletRequest request, String memberCode, long refId) {
		...
		try {
			...
			if (Utility.isNotNull(userInfo.getMemberCode())) {
				query = new Document();
				query.append("$and", Arrays.asList(new Document().append("code", memberCode),
						new Document().append("code", userInfo.getMemberCode())));
			}
	...
	public UserDTO getMemberMasterUserDetail(HttpServletRequest request, String memberCode, long refId) {
		...
		try {
			...
			if (Utility.isNotNull(userInfo.getMemberCode())) {
				...
				matchQuery.append("$and", Arrays.asList(new Document().append("code", memberCode),
						new Document().append("code", userInfo.getMemberCode())));
			}

			List<? extends Bson> pipeline = Arrays.asList(new Document().append("$match", matchQuery),
					new Document().append("$unwind", new Document().append("path", "$users")),
					new Document().append("$match",
							new Document().append("users.username", Constant.MEMBER_MASTER_USER_PREFIX + memberCode)),
	...
	public void createMemberFunctions(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			FunctionsDTO memberDto = Utility.getGson().fromJson(pendingApproval.getPendingData().getPendingValue(),
					FunctionsDTO.class);
	...
	public void createMemberFunctionsPA(HttpServletRequest request, String memberCode, ApprovalFunctionsDTO memberDto,
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request, ActivityLogService.ACTIVITY_CREATE_MEMBER_FUNCTIONS,
					ActivityLogService.ACTIVITY_CREATE_MEMBER_FUNCTIONS_DESC, String.valueOf(memberCode), approvalId);
		} catch (CustomException e) {
			throw e;
		} catch (Exception e) {
			AMLogger.logError(className, methodName, refId, e);
	...
	private String insertMemberFunctionsAssignPA(UserInfoDTO userInfo, String memberCode,
			ApprovalFunctionsDTO memberDto, long refId) {
	...
	public void assignCommodities(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			CommoditiesDTO memberDto = Utility.getGson().fromJson(pendingApproval.getPendingData().getPendingValue(),
					CommoditiesDTO.class);
			// get redis user info
			UserInfoDTO userInfo = Utility.getRedisUserInfo(template, Utility.getAccessToken(request), refId);
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
	...
	public void setCommoditiesPositionLimit(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			CommoditiesDTO pendingCommDto = Utility.getGson()
					.fromJson(pendingApproval.getPendingData().getPendingValue(), CommoditiesDTO.class);
	...
	public void createMemberCommoditiesPA(HttpServletRequest request, String memberCode,
			ApprovalCommoditiesDTO memberDto, long refId) {
		...
		try {
			...
			if (Utility.isNotNull(memberDto.getType())) {
				...
				if (Constant.COMMODITIES_TYPE_ASSIGN.equals(memberDto.getType())) {
					...
					String approvalId = insertMemberCommoditiesPA(userInfo, memberCode, memberDto, serviceFunctionName,
							functionCode, functionName, refId);
					// send activity log
					activityLogService.sendActivityLog(userInfo, request,
							ActivityLogService.ACTIVITY_CREATE_MEMBER_COMMODITIES_ASSIGN,
							ActivityLogService.ACTIVITY_CREATE_MEMBER_COMMODITIES_ASSIGN_DESC, memberCode, approvalId);
					...
					activityLogService.sendActivityLog(userInfo, request,
							...
							ActivityLogService.ACTIVITY_CREATE_MEMBER_COMMODITIES_FEE_DESC, memberCode, approvalId);
				}
	...
	private String insertMemberCommoditiesPA(UserInfoDTO userInfo, String memberCode, ApprovalCommoditiesDTO memberDto,
			String serviceFunctionName, String functionCode, String functionName, long refId) {
	...
	public void createOrderLimit(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			OrderLimitDTO memberDto = Utility.getGson().fromJson(pendingApproval.getPendingData().getPendingValue(),
					OrderLimitDTO.class);
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_ORDER_LIMIT,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_ORDER_LIMIT_DESC, memberCode,
					pendingApproval.getId());
				...
				if (Utility.isCQGSyncOn()) {
					...
					if (investors != null) {
						for (Investor inv : investors) {
							boolean result = cqgService.updateCQGRiskParamsTSL(inv.getCqgInfo().getAccountId(),
									memberDto.getOrderLimit(), refId);
							if (!result) {
								AMLogger.logMessage(className, methodName, refId,
										"Cannot update trade size limit for " + inv.getInvestorCode());
							}
						}
					}
				}
	...
	public void createDefaultPositionLimit(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			DefaultPositionLimitDTO memberDto = Utility.getGson()
					.fromJson(pendingApproval.getPendingData().getPendingValue(), DefaultPositionLimitDTO.class);

			// get redis user info
			UserInfoDTO userInfo = Utility.getRedisUserInfo(template, Utility.getAccessToken(request), refId);
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_DEFAULT_POSITION_LIMIT,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_DEFAULT_POSITION_LIMIT_DESC, memberCode,
					pendingApproval.getId());
	...
	public void createOrderLimitPA(HttpServletRequest request, String memberCode, ApprovalOrderLimitDTO memberDto,
			long refId) {
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_CREATE_MEMBER_DEFAULT_SETTING,
	...
	public void createDefaultPositionLimitPA(HttpServletRequest request, String memberCode,
			ApprovalDefaultPositionLimitDTO memberDto, long refId) {
	...
	private String insertDefaultPositionLimitSettingPA(UserInfoDTO userInfo, String memberCode,
			ApprovalDefaultPositionLimitDTO memberDto, long refId) {
	...
	public void setMemberNewPositionOrderLock(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			RiskParametersDTO memberDto = Utility.getGson().fromJson(pendingApproval.getPendingData().getPendingValue(),
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_RISK_NEW_POSITION_ORDER_LOCK,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_RISK_NEW_POSITION_ORDER_LOCK_DESC, memberCode,
					pendingApproval.getId());

			Document updateDocument = new Document();
			updateDocument.append("riskParameters.newPositionOrderLock",
					memberDto.getRiskParameters().getNewPositionOrderLock());
			updateDocument.append("lastModifiedUser", Utility.getCurrentUsername());
			updateDocument.append("lastModifiedDate", System.currentTimeMillis());
	...
	public void setMemberNewPositionOrderLockPA(HttpServletRequest request, String memberCode,
			ApprovalRiskParametersDTO memberDto, long refId) {
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_CREATE_MEMBER_RISK_NEW_POSITION_ORDER_LOCK,
					ActivityLogService.ACTIVITY_CREATE_MEMBER_RISK_NEW_POSITION_ORDER_LOCK_DESC, memberCode,
					approvalId);
		} catch (CustomException e) {
			throw e;
		} catch (Exception e) {
	...
	private String insertMemberNewPositionOrderLockPA(UserInfoDTO userInfo, String memberCode,
			ApprovalRiskParametersDTO memberDto, long refId) {
	...
	public void setMemberOrderLock(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_RISK_ORDER_LOCK,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_RISK_ORDER_LOCK_DESC, memberCode,
					pendingApproval.getId());
	...
	public void setMemberOrderLockPA(HttpServletRequest request, String memberCode, ApprovalRiskParametersDTO memberDto,
			long refId) {
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_CREATE_MEMBER_RISK_ORDER_LOCK,
	...
	public void setMemberMarginWithDrawalLock(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_RISK_MARGIN_WITHDRAWAL,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_RISK_MARGIN_WITHDRAWAL_DESC, memberCode,
					pendingApproval.getId());

			Document updateDocument = new Document();
			updateDocument.append("riskParameters.marginWithdrawalLock",
					memberDto.getRiskParameters().getMarginWithdrawalLock());
			updateDocument.append("lastModifiedUser", Utility.getCurrentUsername());
			updateDocument.append("lastModifiedDate", System.currentTimeMillis());
	...
	public void setMemberMarginWithDrawalLockPA(HttpServletRequest request, String memberCode,
			ApprovalRiskParametersDTO memberDto, long refId) {
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_CREATE_MEMBER_RISK_MARGIN_WITHDRAWAL,
	...
	private String insertMemberMarginWithDrawalLockPA(UserInfoDTO userInfo, String memberCode,
			ApprovalRiskParametersDTO memberDto, long refId) {
	...
	public void setMarginMultiplierBulk(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			MarginMultiplierDTO memberDto = Utility.getGson()
					.fromJson(pendingApproval.getPendingData().getPendingValue(), MarginMultiplierDTO.class);

			// get redis user info
			UserInfoDTO userInfo = Utility.getRedisUserInfo(template, Utility.getAccessToken(request), refId);
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_MARGIN_MULTIPLIER_BULK,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_MARGIN_MULTIPLIER_BULK_DESC, memberCode,
					pendingApproval.getId());
			...
			if (Utility.isCQGSyncOn()) {
				...
				if (investors != null) {
					for (Investor inv : investors) {
						boolean result = cqgService.updateCQGRiskParamsMM(inv.getCqgInfo().getAccountId(),
								memberDto.getMarginMultiplier(), refId);
						if (!result) {
							AMLogger.logMessage(className, methodName, refId,
									"Cannot update trade size limit for " + inv.getInvestorCode());
						}
	...
	public void setMarginMultiplierBulkPA(HttpServletRequest request, String memberCode,
			ApprovalMarginMultiplierDTO memberDto, long refId) {
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_CREATE_MEMBER_MARGIN_MULTIPLIER_BULK,
	...
	private String insertMemberMarginMultiplierBulkPA(UserInfoDTO userInfo, String memberCode,
			ApprovalMarginMultiplierDTO memberDto, long refId) {
		String methodName = "insertMemberMarginWithDrawalLockPA";
		String approvalId = "";
		try {
	...
	public void setMarginRatioAlertBulk(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			MarginRatioAlertDTO memberDto = Utility.getGson()
					.fromJson(pendingApproval.getPendingData().getPendingValue(), MarginRatioAlertDTO.class);

			// get redis user info
			UserInfoDTO userInfo = Utility.getRedisUserInfo(template, Utility.getAccessToken(request), refId);
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_MARGIN_RATIO_BULK,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_MARGIN_RATIO_BULK_DESC, memberCode,
					pendingApproval.getId());
	...
	public void setMarginRatioAlertBulkPA(HttpServletRequest request, String memberCode,
			ApprovalMarginRatioAlertDTO memberDto, long refId) {
		...
		try {
			...
			if (memberDto.getPendingData().getMarginRatioAlert().getFinalizationRatio() >= memberDto.getPendingData()
					.getMarginRatioAlert().getCancelOrderRatio()
					|| (memberDto.getPendingData().getMarginRatioAlert().getFinalizationRatio() >= memberDto
							.getPendingData().getMarginRatioAlert().getWarningRatio())) {
				isValidRatio = false;
			} else if (memberDto.getPendingData().getMarginRatioAlert().getCancelOrderRatio() >= memberDto
					.getPendingData().getMarginRatioAlert().getWarningRatio()) {
				isValidRatio = false;
			}
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_CREATE_MEMBER_MARGIN_RATIO_BULK,
	...
	private String insertMarginRatioAlertBulkPA(UserInfoDTO userInfo, String memberCode,
			ApprovalMarginRatioAlertDTO memberDto, long refId) {
	...
	public void setGeneralFeeBulk(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			GeneralFeeDTO generalFeeDto = Utility.getGson().fromJson(pendingApproval.getPendingData().getPendingValue(),
					GeneralFeeDTO.class);
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_GENERAL_FEE_BULK,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_GENERAL_FEE_BULK_DESC, memberCode,
					pendingApproval.getId());
	...
	public void setGeneralFeesBulkPA(HttpServletRequest request, String memberCode, GeneralFeeDTO generalFeeDto,
			long refId) {
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_CREATE_MEMBER_GENERAL_FEE_BULK,
	...
	public void updateGeneralFeesBulkPA(HttpServletRequest request, String memberCode,
			ApprovalGeneralFeeDTO generalFeeDto, long refId) {
	...
	private String insertGeneralFeesBulkUpdatePA(UserInfoDTO userInfo, String memberCode,
			ApprovalGeneralFeeDTO generalFeeDto, long refId) {
	...
	public void setBrokerCommoditiesFeeBulk(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			CommodityFeesDTO memberDto = Utility.getGson().fromJson(pendingApproval.getPendingData().getPendingValue(),
					CommodityFeesDTO.class);

			// get redis user info
			UserInfoDTO userInfo = Utility.getRedisUserInfo(template, Utility.getAccessToken(request), refId);
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_BROKER_COMMODITIES_FEE_BULK,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_BROKER_COMMODITIES_FEE_BULK_DESC, memberCode,
					pendingApproval.getId());
			...
			for (CommodityFee commFee : memberDto.getCommodityFees()) {
				...
				CommodityFee currentCommodityFee = getCommodityFeeInAListByCode(currentCommodityFees,
						commFee.getCommodityCode());
			...
			try (MongoCursor<Document> cur = brokerCollection.find(brokerQuery).projection(projection).iterator()) {
				...
				while (cur.hasNext()) {
					...
					if (brokerDto != null && brokerDto.getCommodities() != null
							&& brokerDto.getCommodities().size() > 0) {
						...
						for (BrokerCommodity comm : brokerDto.getCommodities()) {
							...
							CommodityFee newComm = getCommodityFeeInAListByCode(memberDto.getCommodityFees(),
									comm.getCommodityCode());
	...
	public void setBrokerCommoditiesFeeBulkPA(HttpServletRequest request, String memberCode,
			ApprovalCommodityFeesDTO memberDto, long refId) {
		String methodName = "setBrokerCommoditiesFeeBulk";
		try {
			if (!memberRepository.existsMemberByCode(memberCode)) {
				throw new CustomException(ErrorMessage.RESULT_NOT_FOUND, HttpStatus.OK);
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_CREATE_MEMBER_COMMODITIES_FEE_BULK,
	...
	private String insertBrokerCommoditiesFeeBulkPA(UserInfoDTO userInfo, String memberCode,
			ApprovalCommodityFeesDTO memberDto, long refId) {
	...
	public void setInvestorCommoditiesFeeBulk(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog(userInfo, request,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_INVESTOR_COMMODITIES_FEE_BULK,
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_INVESTOR_COMMODITIES_FEE_BULK_DESC, memberCode,
					pendingApproval.getId());
			...
			try (MongoCursor<Document> cur = investorCollection.find(investorQuery).projection(projection).iterator()) {
				...
				while (cur.hasNext()) {
					...
					if (investorDto != null && investorDto.getCommodities() != null
							&& investorDto.getCommodities().size() > 0) {
	...
	public void setInvestorCommoditiesFeeBulkPA(HttpServletRequest request, String memberCode,
			ApprovalCommodityFeesDTO memberDto, long refId) {
	...
	private String insertInvestorCommoditiesFeeBulkPA(UserInfoDTO userInfo, String memberCode,
			ApprovalCommodityFeesDTO memberDto, long refId) {
		...
		try {
			...
			pendingApproval.setApiUrl(String.format(ApprovalConstant.APPROVAL_PENDING_URL, approvalId));
			pendingApproval
					.setFunctionCode(SystemFunctionCode.APPROVAL_MEMBER_INVESTOR_COMMODITIES_FEE_BULK_CREATE_CODE);
	...
	public BasePagination<UserDTO> listMemberUsers(HttpServletRequest request, String memberCode, long refId) {
		...
		try {
			...
			if (Utility.isNotNull(userInfo.getMemberCode())) {
				query1 = new Document();
				query1.append("$and", Arrays.asList(new Document().append("code", memberCode),
						new Document().append("code", userInfo.getMemberCode())));
			}
			...
			List<? extends Bson> pipeline = Arrays
					.asList(new Document().append("$match", query1),
							...
							new Document().append("$match",
									new Document().append("users.username",
											new Document().append("$ne", Constant.MEMBER_MASTER_USER_PREFIX
													+ memberCode))),
	...
	public void createMemberUser(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			UserDTO memberUserDto = Utility.getGson().fromJson(pendingApproval.getPendingData().getPendingValue(),
					UserDTO.class);
			...
			activityLogService.sendActivityLog2(userInfo, request,
					...
					ActivityLogService.ACTIVITY_APPROVAL_CREATE_MEMBER_USER_DESC, memberUserDto.getUsername(),
					memberCode, pendingApproval.getId());
			...
			String password = Utility.generateRandomPassword();
			String pin = Utility.generateRandomPin();
			LoginAdminUser newLoginAdmUser = createLoginAdminUser(memberCode, memberDto.getName(), memberUserDto,
					password, pin, refId);

			// send email
			if (Utility.isNotifyOn()) {
				Utility.sendCreateNewUserEmail(Constant.MEMBER_MASTER_USER_PREFIX, "", newLoginAdmUser.getUsername(),
						password, pin, refId);
			}
		} catch (Exception e) {
			AMLogger.logError(className, methodName, refId, e);
	...
	public void createMemberUserPA(HttpServletRequest request, String memberCode, UserDTO memberUserDto, long refId) {
		...
		if (!existedUser) {
			try {
				...
				// send activity log
				activityLogService.sendActivityLog2(userInfo, request, ActivityLogService.ACTIVITY_CREATE_MEMBER_USER,
						ActivityLogService.ACTIVITY_CREATE_MEMBER_USER_DESC, memberUserDto.getUsername(), memberCode,
						approvalId);
			} catch (Exception e) {
				AMLogger.logError(className, methodName, refId, e);
				throw new CustomException(ErrorMessage.ERROR_OCCURRED, HttpStatus.INTERNAL_SERVER_ERROR);
	...
	private String insertMemberUserCreatePA(UserInfoDTO userInfo, String memberCode, UserDTO memberUserDto,
		...
		try {
			...
			pendingApproval.setDescription(SystemFunctionCode.getApprovalDescription2(
					SystemFunctionCode.MEMBER_USER_CREATE_DESC, memberUserDto.getUsername(), memberCode));
			pendingApproval.setStatus(Constant.APPROVAL_STATUS_PENDING);
			pendingApproval.setNestedObjInfo(nestedObjInfo);
			pendingApproval.setPendingData(pendingData);
			pendingApproval.setSessionDate(Utility.getSessionDateRedis(template));
	...
	public void updateMemberUser(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			...
			UpdateUserDTO userDto = Utility.getGson().fromJson(pendingApproval.getPendingData().getPendingValue(),
					UpdateUserDTO.class);
			...
			// send activity log
			activityLogService.sendActivityLog2(userInfo, request,
					ActivityLogService.ACTIVITY_APPROVAL_UPDATE_MEMBER_USER,
					ActivityLogService.ACTIVITY_APPROVAL_UPDATE_MEMBER_USER_DESC, username, memberCode,
					pendingApproval.getId());

			boolean isLoginUserUpdated = false;

			BasicDBObject newDocument = new BasicDBObject();
			BasicDBObject newLoginUserDoc = new BasicDBObject();

			if (Utility.isNotNull(userDto.getFullName())) {
				newDocument.append("users.$.fullName", userDto.getFullName());
				newLoginUserDoc.append("fullName", userDto.getFullName());
				isLoginUserUpdated = true;
			}
				
			if (Utility.isNotNull(userDto.getPhoneNumber())) {
				newDocument.append("users.$.phoneNumber", userDto.getPhoneNumber());
				newLoginUserDoc.append("phoneNumber", userDto.getPhoneNumber());
				isLoginUserUpdated = true;
			}
				
			if (Utility.isNotNull(userDto.getEmail())) {
				newDocument.append("users.$.email", userDto.getEmail());
				newLoginUserDoc.append("email", userDto.getEmail());
				isLoginUserUpdated = true;
			}
				
			if (Utility.isNotNull(userDto.getStatus())) {
				newDocument.append("users.$.status", userDto.getStatus().toUpperCase());
				newLoginUserDoc.append("status", userDto.getStatus().toUpperCase());
				isLoginUserUpdated = true;
			}
			...
			if (newDocument.isEmpty()) {
				...
				if (isLoginUserUpdated) {
					// update login admin user
					MongoCollection<Document> loginAdmUserCollection = database.getCollection("login_admin_users");

					BasicDBObject loginUserQuery = new BasicDBObject();
					loginUserQuery.append("memberCode", memberCode);
					loginUserQuery.append("username", username);

					BasicDBObject loginUserUpdate = new BasicDBObject();
					loginUserUpdate.append("$set", newLoginUserDoc);

					loginAdmUserCollection.updateOne(loginUserQuery, loginUserUpdate);
				}
			}
		} catch (CustomException e) {
			throw e;
	...
	private String insertMemberUserUpdatePA(UserInfoDTO userInfo, String memberCode, String username,
		...
		try {
			...
			pendingApproval.setDescription(SystemFunctionCode
					.getApprovalDescription2(SystemFunctionCode.MEMBER_USER_CREATE_DESC, username, memberCode));
			pendingApproval.setStatus(Constant.APPROVAL_STATUS_PENDING);
			pendingApproval.setNestedObjInfo(nestedObjInfo);
			pendingApproval.setPendingData(pendingData);
			pendingApproval.setSessionDate(Utility.getSessionDateRedis(template));
	...
	public UserDTO getMemberUser(HttpServletRequest request, String memberCode, String username, long refId) {
		...
		try {
			...
			List<? extends Bson> pipeline = Arrays.asList(new Document().append("$match", query),
					new Document().append("$unwind", new Document().append("path", "$users")),
	...
	public void saveMemberUserRoles(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			String username = pendingApproval.getPendingData().getQueryValue2();
			UserRolesDTO userDto = Utility.getGson().fromJson(pendingApproval.getPendingData().getPendingValue(),
					UserRolesDTO.class);
	...
	public void saveMemberUserRolesPA(HttpServletRequest request, String memberCode, String username,
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog2(userInfo, request, ActivityLogService.ACTIVITY_CREATE_MEMBER_USER_ROLES,
					ActivityLogService.ACTIVITY_CREATE_MEMBER_USER_ROLES_DESC, username, memberCode, approvalId);
		} catch (CustomException e) {
			throw e;
		} catch (Exception e) {
			AMLogger.logError(className, methodName, refId, e);
	...
	private String insertMemberUserRoleCreatePA(UserInfoDTO userInfo, String memberCode, String username,
		...
		try {
			...
			pendingApproval.setDescription(SystemFunctionCode.getApprovalDescription2(
					SystemFunctionCode.MEMBER_USER_ROLES_ASSIGN_CREATE_DESC, username, memberCode));
			pendingApproval.setStatus(Constant.APPROVAL_STATUS_PENDING);
			pendingApproval.setNestedObjInfo(nestedObjInfo);
			pendingApproval.setPendingData(pendingData);
			pendingApproval.setSessionDate(Utility.getSessionDateRedis(template));
	...
	public void saveMemberUserFunctions(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			String memberCode = pendingApproval.getPendingData().getQueryValue();
			String username = pendingApproval.getPendingData().getQueryValue2();
			FunctionsDTO userDto = Utility.getGson().fromJson(pendingApproval.getPendingData().getPendingValue(),
					FunctionsDTO.class);
	...
	public void saveMemberUserFunctionsPA(HttpServletRequest request, String memberCode, String username,
		...
		try {
			...
			activityLogService.sendActivityLog2(userInfo, request,
					...
					ActivityLogService.ACTIVITY_CREATE_MEMBER_USER_FUNCTIONS_DESC, username, memberCode, approvalId);
		} catch (CustomException e) {
			throw e;
		} catch (Exception e) {
			AMLogger.logError(className, methodName, refId, e);
	...
	private String insertMemberUserFunctionsCreatePA(UserInfoDTO userInfo, String memberCode, String username,
		...
		try {
			...
			pendingApproval.setDescription(SystemFunctionCode.getApprovalDescription2(
					SystemFunctionCode.MEMBER_USER_FUNCTIONS_ASSIGN_CREATE_DESC, username, memberCode));
			pendingApproval.setStatus(Constant.APPROVAL_STATUS_PENDING);
			pendingApproval.setNestedObjInfo(nestedObjInfo);
			pendingApproval.setPendingData(pendingData);
			pendingApproval.setSessionDate(Utility.getSessionDateRedis(template));
		...
		try {
			...
			List<? extends Bson> pipeline = Arrays.asList(new Document().append("$match", query),
					...
					new Document().append("$project", new Document().append("_id", 0.0)
							.append("roleFunctions", "$roleObj.functions").append("specificFunctions", "$functions")));
			...
			List<RoleFunction> allFunctionsWithoutDuplicates = new ArrayList<RoleFunction>(
					new HashSet<RoleFunction>(allFunctions));
	...
	public List<ListElementDTO> getMemberBrokerList(HttpServletRequest request, String memberCode, long refId) {
		...
		if (Utility.isNotNull(userInfo.getMemberCode())) {
			query = new Document();
			query.append("$and", Arrays.asList(new Document().append("memberCode", memberCode),
					new Document().append("memberCode", userInfo.getMemberCode())));
		}
	...
	public void moveAllInvestorsToNewMember(HttpServletRequest request, PendingApproval pendingApproval, long refId) {
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog2(userInfo, request,
					ActivityLogService.ACTIVITY_APPROVAL_MEMBER_MOVE_ALL_INVESTORS,
					ActivityLogService.ACTIVITY_APPROVAL_MEMBER_MOVE_ALL_INVESTORS_DESC, memberCode,
	...
	public void moveAllInvestorsToNewMemberPA(HttpServletRequest request, ApprovalChangeGroupDTO changeGroupDto,
			long refId) {
		...
		try {
			...
			// send activity log
			activityLogService.sendActivityLog2(userInfo, request,
					ActivityLogService.ACTIVITY_MEMBER_MOVE_ALL_INVESTORS,
					ActivityLogService.ACTIVITY_MEMBER_MOVE_ALL_INVESTORS_DESC,
					changeGroupDto.getOldData().getGroupCode(), changeGroupDto.getPendingData().getGroupCode(),
					approvalId);
		} catch (CustomException e) {
			throw e;
		} catch (Exception e) {
	...
	private String insertMoveAllInvestorsToNewMemberPA(UserInfoDTO userInfo, ApprovalChangeGroupDTO changeGroupDto,
			long refId) {
		...
		try {
			...
			pendingApproval.setDescription(SystemFunctionCode.getApprovalDescription2(
					SystemFunctionCode.MEMBER_MOVE_ALL_INVESTORS_DESC, changeGroupDto.getOldData().getGroupCode(),
					changeGroupDto.getPendingData().getGroupCode()));
			pendingApproval.setStatus(Constant.APPROVAL_STATUS_PENDING);
			pendingApproval.setNestedObjInfo(nestedObjInfo);
			pendingApproval.setPendingData(pendingData);
			pendingApproval.setSessionDate(Utility.getSessionDateRedis(template));
